...
class EmotionPlayer:
        ...
        try:
            ...
            while not self.stop_event.is_set():
                ...
                if current_time >= data["time"][-1]:
                    ...
                    self.reachy.send_goal_positions(check_positions=False)
                    logging.info("Reached end of recording normally, starting idle motion.")
                    # (Idle loop code unchanged follows here.)
                    break
    ...
    def _replay_thread_smart_interpol_my_antenna_example(self, filename: str):
        logging.info("Starting emotion playback for %s", filename)
        if self.reachy is None:
            logging.error("No valid Reachy instance available.")
            return
        ...
        l_antenna_goal_position = self.reachy.head.l_antenna.present_position
        ...
        try:
            while not self.stop_event.is_set():
                ...
                l_antenna_goal_position_recording = lerp(data["l_antenna"][idx_prev], data["l_antenna"][idx_next], alpha)
                index = bisect.bisect_right(data["time"], current_time+dt)
                logging.debug(f"index: {index}, expected index: {current_time/dt:.0f}")
                idx_prev = index - 1 if index > 0 else 0
                idx_next = index if index < len(data["time"]) else idx_prev
                t_prev = data["time"][idx_prev]
                t_next = data["time"][idx_next]
                l_antenna_goal_position_recording_next = lerp(data["l_antenna"][idx_prev], data["l_antenna"][idx_next], alpha)
                # This is the "movement" requested in the recording
                l_antenna_recording_speed = (l_antenna_goal_position_recording_next - l_antenna_goal_position_recording) / dt
                # But we must also try to get to the recording position at the right time. Calculate the speed needed to get there and limiting the speed
                l_antenna_interpol_speed = np.clip((l_antenna_goal_position_recording - l_antenna_goal_position) / dt, -self.max_joint_speed, self.max_joint_speed)
                l_antenna_goal_position = l_antenna_goal_position + l_antenna_interpol_speed * dt + l_antenna_recording_speed * dt
                self.reachy.head.l_antenna.goal_position = l_antenna_goal_position
